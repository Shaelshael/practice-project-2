...
'''mermaid
graph LR;
   Файл создан (статус — untracked)  --> Команда git add --> Статус staged --> Файл запомнен Git'ом и отслеживается --> В файл внесены изменения --> Статус modified --> Файл подтверждён через git add --> Статус снова staged --> Файл запомнен Git'ом и изменён --> Коммитим файл --> Статус commited ();  
'''
